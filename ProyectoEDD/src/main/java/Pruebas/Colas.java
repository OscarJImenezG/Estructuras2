...
public class Colas {
    ...
    private Colas cola;
    private int cima, inicio;
    private static final int TAMANIO_ARREGLO = 2;
    ...
    public void enqueue(int entrada) {
        if (colas[cima] == null && cima < TAMANIO_ARREGLO) {
            colas[cima] = entrada;
            if (cima < TAMANIO_ARREGLO) {
                cima++;
            }
        } else if (colas[cima] != null && colas[0] != null) {
            JOptionPane.showMessageDialog(null, "La cola esta llena", "Cola llena", JOptionPane.OK_OPTION);
        } else if (cima < TAMANIO_ARREGLO) {
            cima = 0;
            if (colas[cima] != null) {
                colas[cima] = entrada;
                cima++;
    ...
    public void dequeue() {
        if (colas[inicio] != null && inicio < TAMANIO_ARREGLO) {
            colas[inicio] = null;
            inicio++;
        } else if (colas[inicio] == null) {
            JOptionPane.showMessageDialog(null, "La cola esta vacia", "Cola vacia", JOptionPane.OK_OPTION);
        } else if (inicio == TAMANIO_ARREGLO - 1) {
            inicio = 0;
