...
public class Colas {
    ...
    public JPanel manejoDeCola() {
        ...
        btnColas.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                ...
                JButton btnLimpiar = new JButton("Limpiar la cola");
                ...
                btnLimpiar.setForeground(new Color(213, 159, 15));
                btnLimpiar.setBackground(new Color(0, 43, 121));
                ...
                btnLimpiar.addMouseListener(new MouseAdapter() {
                    @Override
                    public void mouseEntered(MouseEvent e) {
                        btnLimpiar.setBackground(new Color(213, 159, 15));
                        btnLimpiar.setForeground(new Color(0, 43, 121));
                    }
    ...
    public JPanel tablaHorizontal(String[] arreglo, JPanel tmp) {
        ...
        tabla.setSize(600, 150);
        temp.add(tituloLabel);
        temp.setPreferredSize(new Dimension(600, 150));
        temp.add(tabla);
        JScrollPane redimesion = new JScrollPane(temp);
        redimesion.setBorder(null);
    ...
    public JPanel tablaVertical(String[] arreglo, JPanel tmp) {
        ...
        JScrollPane[] redimension = new JScrollPane[10];
        tituloLabel = new JLabel("Operaciones con una Pila", SwingConstants.CENTER);
        tituloLabel.setFont(new Font("Arial", Font.BOLD, 20));
        tituloLabel.setForeground(Color.BLACK);
        for (int i = 0; i < 10; i++) {
            arreg[i] = new JPanel(new FlowLayout());
            arreg[i].setBorder(new LineBorder(Color.BLACK, 2));
            redimension[i] = new JScrollPane(arreg[i]);
            redimension[i].setBorder(null);
            redimension[i].setPreferredSize(new Dimension(100, 45));
            tabla.add(redimension[i], i);
        }
        byte contador = 9;
        for (int i = 0; i < arreglo.length; i++) {
            JLabel[] lbls = new JLabel[arreglo.length];
            lbls[i] = new JLabel(arreglo[i]);
            lbls[i].setForeground(Color.BLACK);
            System.out.println("Agrngamos a la IG" + arreglo[i]);
            arreg[contador].add(lbls[i]);
            contador--;
        }
        temp.add(tituloLabel, BorderLayout.NORTH);
        temp.add(tabla, BorderLayout.CENTER);
        JScrollPane redim = new JScrollPane(temp);
        tmp.add(redim, BorderLayout.WEST, 0);
        redim.setPreferredSize(new Dimension(300, 500));
        tabla.setPreferredSize(new Dimension(300, 100));
        redim.setBorder(null);
        return tmp;
    }
}
